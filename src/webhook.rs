...
use ser::{ExecuteWebhook, Mentionable};
...
impl Display for WebhookErrorContext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            GettingWebhooks(channel) => write!(
                f,
                "getting webhooks for channel {channel_mention}",
                channel_mention = channel.mention()
            ),
            CreatingWebhook(channel) => write!(
                f,
                "creating a webhook in channel {channel_mention}",
                channel_mention = channel.mention()
            ),
            ExecutingWebhook(webhook) => write!(f, "executing webhook {webhook}"),
        }
    }
}
...
impl WebhookExecutor {
        ...
        R: WebhookExecutionResult,
    {
        ...
        for i in 2..=4 {
                ...
                let part = match attachment {
                    ...
                    ser::AttachmentType::Image(url) => {
                        ...
                        let mut part = match response.content_length() {
                            Some(len) => {
                                reqwest::multipart::Part::stream_with_length(response, len)
                            }
            ...
            let mut query: Vec<(Cow<str>, Cow<str>)> = vec![(
                "wait".into(),
                if R::wait() {
                    "true".into()
                } else {
