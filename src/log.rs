...
use crate::errors::{Contextualizable, Error, Result};
...
pub const GET_LOGCH: &str =
    "SELECT channel_id FROM log_channels WHERE server_id = $1 AND log_typ = $2;";
...
impl Display for LogError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NoLogChannel => write!(f, "no log channel set up for log type"),
            Self::InvalidLogType(t) => write!(f, "invalid log type: {t}"),
        }
    }
}
...
async fn set_logch(
    ...
    trans
        .execute(
            SET_LOGCH,
            &[&guild.0.to_be_bytes(), &log_type, &channel.0.to_be_bytes()],
        )
        .await?;
    trans.commit().await?;
    Ok(())
...
async fn del_logch(data: &crate::Data, guild: ser::GuildId, log_type: LogType) -> Result<()> {
    let mut conn = data.dbconn.get().await?;
    let trans = conn.transaction().await?;
    trans
        .execute(DEL_LOGCH, &[&guild.0.to_be_bytes(), &log_type])
        .await?;
    trans.commit().await?;
    Ok(())
}
...
where
    F: for<'a> FnOnce(&'a mut ser::CreateEmbed) -> &'a mut ser::CreateEmbed + Send,
{
    ...
    logch
        .send_message(http, |m| {
            m.embed(|e| builder(e.title(log_type.title()).color(log_type.color())))
...
{
    let message = post_log(ctx, ctx.data(), guild, LogType::Purge, |e| {
...
pub async fn log_message_edit<'a>(
    ...
    let Some(_monguard) = fwctx.user_data.logdata.try_monopolize([evt.id]) else {
        return Ok(());
    };
    if evt.author.as_ref().is_some_and(|a| user_is_self(fwctx, a)) {
        return Ok(());
    }
    if evt
        .edited_timestamp
        .is_none_or(|t| *t < Utc::now() - Duration::minutes(5))
    {
        return Ok(());
    }
    post_log(
...
pub async fn log_message_delete(
    ...
    let Some(_monguard) = data.logdata.try_monopolize([deleted_msg_id]) else {
        return Ok(());
    };

    let cached = client
        .cache()
        .and_then(|c| c.message(channel_id, deleted_msg_id));
    post_log(
        client.http(),
        data,
        guild_id.unwrap(),
...
pub async fn log_user_ban(
    ...
    post_log(client.http(), data, guild_id, LogType::UserBan, |e| {
...
pub async fn log_user_join(
    ...
    post_log(http, data, new_member.guild_id, LogType::UserJoin, |e| {
        e.author(|a| user_to_embed_author(&new_member.user, a))
            .field("User", new_member.user.mention(), true)
            .field(
                "Account created",
                fmttime_discord(new_member.user.created_at()),
                true,
            )
    })
    .await?;
...
pub async fn log_user_remove(
    ...
    post_log(client.http(), data, guild_id, log_type, |e| {
...
pub async fn log_voice_update(
    ...
    let loggable_transitions = match (old_state, new_state) {
        ...
        (
            Some(ser::VoiceState {
                guild_id: Some(old_guid),
                channel_id: old_chid,
                ..
            }),
            ser::VoiceState {
                guild_id: Some(new_guid),
                channel_id: new_chid,
                ..
            },
        ...
        ) if old_guid != new_guid => vec![
            (*old_guid, Some(*old_chid), None),
            (*new_guid, None, Some(*new_chid)),
        ],
        _ => vec![],
    };
    for (guild_id, old_vc, new_vc) in loggable_transitions {
        post_log(http.as_ref(), data, guild_id, LogType::VoiceUpdate, |e| {
            let [old_vc_str, new_vc_str] = [old_vc, new_vc].map(|vc_opt| match vc_opt {
...
pub async fn log_event<'a>(
    ...
    match evt {
        ...
        GuildMemberAddition { new_member } => log_user_join(ctx, data, new_member).await?,
        GuildMemberRemoval {
            guild_id,
            user,
            member_data_if_available,
        } => log_user_remove(ctx, data, *guild_id, user, member_data_if_available).await?,
        VoiceStateUpdate { old, new } => log_voice_update(ctx, data, new, old).await?,
        _ => {}
    }
    Ok(())
}
...
pub async fn log_modmail(
    ...
    post_log(ctx.http(), data, guild, LogType::Modmail, |e| {
        e.field("User", user.mention(), true)
...
pub async fn log(
    ctx: PoiseContext<'_>,
    #[description = "Type of events to log"] log_type: LogType,
    ...
    ctx.defer().await.map_err(Error::from)?;
    match channel {
        Some(chid) => {
            set_logch(ctx.data(), guild_id, log_type, chid)
                .await
                .contextualize(LogErrorContext::SettingLogChannel(guild_id, log_type, chid))?;
            ctx.say(format!(
                "Successfully set the channel for `{log_type}` to {channel_mention}",
