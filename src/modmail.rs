...
impl Modmail {
    const LOOKUP_COMMAND: &str = "SELECT ticket_num, modmailch, modmailmsg, staff_role FROM modmail WHERE guild_id = $1";
    pub async fn lookup<C: GenericClient>(client: &C, gid: ser::GuildId) -> Result<Option<Self>> {
        let row = client.query_opt(Self::LOOKUP_COMMAND, &[&u64::from(gid).to_be_bytes()]).await?;
        let row = match row {
            Some(r) => Some({
                Self {
    ...
    pub async fn update(&self, gid: ser::GuildId, trans: &Transaction<'_>) -> Result<()> {
        let Self { ticket_num, modmailch, modmailmsg, staff_role } = self;
        trans.execute(Self::UPDATE_COMMAND, &[
...
pub async fn modmail(
    ...
    let msg = modmailch.send_message(ctx, |r| {
    ...
    let settings = match Modmail::lookup(&trans, gid).await? {
        Some(m) => {
            if let Err(e) = m.modmailch.delete_message(ctx, m.modmailmsg).await {
                ctx.say(format!("Failed to delete previous modmail message: {e}")).await.logerr();
            }
            Modmail {
...
async fn start_modmail(compinter: &ser::MessageComponentInteraction, data: &crate::Data, ctx: &ser::Context) -> Result<()> {
    let Some(gid) = compinter.guild_id else { return Ok(()) };
    ...
    thread.send_message(ctx, |m| {
        m.content(format_args!("<@{starter_id}> started a new ticket. <@&{staff_role}>",
                               starter_id = compinter.user.id,
                               staff_role = settings.staff_role))
         .components(|c| c.create_action_row(|ar| {
            ar.create_button(|b| {
                b.label("Close")
                 .emoji('🔒')
...
async fn close_modmail(compinter: &ser::MessageComponentInteraction, data: &crate::Data, ctx: &ser::Context) -> Result<()> {
    let Some(gid) = compinter.guild_id else { return Ok(()) };
    let conn = data.dbconn.get().await?;
    let Some(settings) = Modmail::lookup(&*conn, gid).await? else { return Ok(()) };
    let members = compinter.channel_id.get_thread_members(ctx).await?;
    for member in members {
        let Some(uid) = member.user_id else { continue };
        let Ok(user) = uid.to_user(ctx).await else { continue };
        let Ok(has_role) = user.has_role(ctx, gid, settings.staff_role).await else { continue };
        if !has_role {
            compinter.channel_id.remove_thread_member(ctx, uid).await.logerr();
        }
    }
    compinter.edit_original_message(ctx, |m| {
        m.interaction_response_data(|r| {
            r.content("Ticket closed")
             .components(|c| c)
        })
    }).await?;
    compinter.channel_id.edit_thread(ctx, |t| t.archived(true)).await?;
    Ok(())
}
