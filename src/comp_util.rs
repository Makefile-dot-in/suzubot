use crate::{cmd_data, errors::Result, PoiseContext};
use futures::StreamExt;
use itertools::Itertools;
use poise::{serenity_prelude as ser, ChoiceParameter, ReplyHandle};
use std::borrow::Cow;
use std::fmt;
use std::fmt::Debug;
use std::pin::Pin;
...
pub async fn component_test(ctx: PoiseContext<'_>) -> Result<()> {
    ...
    handle
        .edit(ctx, |b| {
            b.components(|c| c).content("indirectly through ctx")
        })
        .await?;
...
pub fn edit<'a>(
    ...
    move |ar| {
        Box::pin(async move {
            handle
                .edit(ctx, |e| {
                    e.content(content).components(|c| c.add_action_row(ar))
                })
                .await?;
...
where
    F: FnOnce(ser::CreateActionRow) -> Fut,
    ...
    ItemGenIt: IntoIterator<Item = Item>,
{
    ...
    let mut pagenum = 0;
    let (items, total, newpagenum) = generator(pagenum).await?;
    pagenum = newpagenum;
    let replyh = ctx
        .send(|m| {
            m.embed(|e| {
                *e = embed(&title, items, pagenum, total);
                e
            })
    ...
    while let Some(interaction) = interactions.next().await {
        ...
        let Ok((items, total, newpagenum)) = generator(pagenum).await else {
            continue;
        };
        pagenum = newpagenum;
