...
const OPTIONS: &[(&str, &[&str], &str)] = &[
    ...
    (
        "migration_info",
        &["MIGRATION"],
        "Views info for a migration.",
    ),
    (
        "migrate",
        &["TARGET"],
        "Migrates the database for the selected profile to TARGET.",
    ),
    (
        "init",
        &[],
        "Initializes the database for the selected profile.",
    ),
...
"
        ...
        let inherent_option_desc = OPTIONS
            .iter()
            .map(|&(param, args, _)| {
                format!(
                    "-{param} {args_joined}",
                    args_joined = args.iter().join(" ")
                )
            })
            .collect::<Vec<_>>();
        let max_inh_option_desc = inherent_option_desc
            .iter()
            .map(String::len)
            .max()
            .unwrap_or(0);
...
fn get_options(
    ...
    while let Some(opt) = iter.next_if(|s| s.starts_with('-')) {
        ...
        let (_, args, _) = OPTIONS
            .iter()
            .find(|&&(opt, _, _)| opt == optname)
            ...
            .map(|(idx, &name)| {
                Ok((
                    ...
                    iter.next().ok_or_else(|| {
                        anyhow!(
                            "-{optname}: expected {args_len} arguments, got {idx}",
                            args_len = args.len()
                        )
                    })?,
                ))
...
> {
    ...
    let exec_name = arg_iter.next().ok_or_else(|| {
        anyhow!("failed to get executable name (try googling for: argv[0] not present)")
    })?;
...
async fn main() {
    ...
    let init_res: Result<(), anyhow::Error> = try {
        ...
        let config: Config = toml::from_str(&config_str).context("error parsing config")?;

        let profile_name_opt = match &position_args[..] {
            [profile] => Some(profile.as_str()),
            [] => None,
            [_, trailing @ ..] => Err(anyhow!(
                "trailing arguments: {trailing_str}",
        ...
        if let Some(migrate_opts) = opt_args.get("migrate") {
            let target = migrate_opts["TARGET"]
                .parse::<u16>()
                .context("parsing migration target")?;
