...
use crate::errors::{AsyncReportErr, Contextualizable, Error, LogError, Result};
...
impl Display for PurgeErrorContext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Deleting(chid) => write!(
                f,
                "deleting messages in {chid_mention}",
...
pub async fn purge(
    ...
    let messages = MessageStream::new(ctx, channelid, before.as_ref().map(|b| b.id))
        ...
        .try_filter(|msg| future::ready(!re.as_ref().is_some_and(|p| !p.is_match(&msg.content))))
        .take(limit.into())
        .try_collect::<Vec<_>>()
        .await?;
    ...
    if messages.len() >= ASK_FOR_CONFIRMATION_ABOVE {
        ...
        match response {
            ...
            _ => {
                handle
                    .edit(ctx, |e| e.content("Purge cancelled.").components(|c| c))
                    .await?;
                return Ok(());
            }
        }
    ...
    handle
        .edit(ctx, |e| e.content("Purging...").components(|c| c))
        .await?;
    ...
    .map_err(|e| {
        e.contextualize(LogErrorContext::Log(
            ctx.guild_id().unwrap(),
            LogType::Purge,
        ))
    });
...
fn first_and_last<T: Copy>(slice: &[T]) -> Option<(T, T)> {
    slice.first().cloned().zip(slice.last().cloned())
}
...
async fn delete_messages(
    ...
    let mut arrch = iter
        .by_ref()
        .take_while(|m| m.created_at() > min_ts)
        .array_chunks::<100>();
    ...
    if let Some(remainder) = arrch.into_remainder() {
        if remainder.len() >= 2 {
            ...
            channelid
                .delete_messages(ctx, remainder_slice.iter().map(Clone::clone))
                .await
                .map_err(|e| errors.push((first_and_last(remainder_slice), e.into())))
                .ok();
        } else {
            for msg in remainder {
                channelid
                    ...
                    .await
                    .map_err(|e| errors.push((Some((msg, msg)), e.into())))
                    .ok();
            }
        }
    }
    ...
    if !errors.is_empty() {
        let errorstr = errors
            .into_iter()
            .map(|(message_range, err)| match message_range {
                Some((m1, m2)) if m1 == m2 => format!(
                    "error purging {m1_link}: {err}",
                    m1_link = m1.link(channelid, ctx.guild_id())
                ),
                Some((m1, m2)) => format!(
                    "error purging {m1_link} to {m2_link}: {err}",
                    m1_link = m1.link(channelid, ctx.guild_id()),
                    m2_link = m2.link(channelid, ctx.guild_id())
                ),
                None => format!("error purging {err}"),
            })
            .join("\n");

        if errorstr.len() <= 1500 {
            ctx.say(format!("Encountered errors while deleting: {errorstr}"))
                .await?;
        } else {
            ctx.send(move |m| {
                m.content("Encountered errors while deleting:").attachment(
                    ser::AttachmentType::Bytes {
    ...
    if let Err(logerr) = logres {
        let response = ask_yn(
            ...
            edit(
                ...
                ctx,
                format!(
                    "⚠️ **WARNING:** logging failed: {logerr}. \
...
pub async fn message_stream_test(
    ...
    for (suzu_msg, ser_msg) in suzu_msgs.into_iter().zip(ser_msgs.into_iter()) {
        if suzu_msg.id != ser_msg.id {
            ctx.say(format!(
                "Stream mismatch ({suzu_msg_id} != {ser_msg_id})",
                suzu_msg_id = suzu_msg.id,
                ser_msg_id = ser_msg.id
            ))
            .await?;
            return Ok(());
        }
