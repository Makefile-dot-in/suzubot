...
impl CustomCommandData {
    pub fn from_command_data<'a>(
        cmd: &'a poise::Command<Data, SuzuError>,
    ) -> &'a CustomCommandData {
        cmd.custom_data
            .downcast_ref::<CustomCommandData>()
            .unwrap_or(&DEF_CMD_DATA)
    }
...
fn ctb_idx_range(s: &str, range: Range<usize>) -> Range<usize> {
    ...
    let Some(diff) = range.end.checked_sub(range.start + 1) else {
        return 0..0;
    };
    let endidx = indices
        .by_ref()
...
fn ts_to_id<Id: From<u64>>(t: ser::Timestamp) -> Id {
    ...
    let millis_since_epoch: u64 = time_since_epoch
        .num_milliseconds()
        .try_into()
        .expect(&format!(
            "are you time travelling to before {epoch_year}? if so, \
