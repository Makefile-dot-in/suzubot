...
use crate::{admin, errors::global_handler::on_error};
use anyhow::anyhow;
    ...
    let framework = poise::Framework::builder()
        .options(poise::FrameworkOptions {
            commands: super::COMMANDS
                ...
                .map(|f| f())
                .filter(|cmd| {
                    profile.test_mode || !CustomCommandData::from_command_data(cmd).test_mode
                })
        ...
        .client_settings(move |c| c.cache_settings(|s| s.max_messages(profile.message_cache_size)))
        .setup(move |ctx, ready, framework| {
            Box::pin(async move {
                ...
                crate::start_services(ctx, ready, framework.shard_manager(), &data);
                Ok(data)
            })
        })
        .build()
