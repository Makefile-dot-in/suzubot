...
fn parse_migrations(
    (input, inter_offset): UnparsedMigrations,
) -> anyhow::Result<Vec<Migration<'static>>> {
    ...
    v.windows(2)
        .map(|sl| {
            ...
            (anum.saturating_add_signed(inter_offset) == bnum)
                .then_some(())
                .ok_or_else(|| {
                    anyhow!("non-sequential migrations: {anum} + ({inter_offset}) != {bnum}")
                })
        })
...
async fn get_last_migration(client: &pg::Client) -> anyhow::Result<u16> {
    ...
    dbint.try_into().map_err(|_| {
        anyhow!(
            "{dbint}: migration number too large. was there an alteration to suzu_table_metadata?"
        )
    })
}
...
pub fn list_migrations() -> anyhow::Result<()> {
    let parsed_migrations = parse_migrations(UP_MIGRATIONS).context("parsing up migrations")?;
...
pub fn print_migration(n: u16) -> anyhow::Result<()> {
    let parsed_up_migrations = parse_migrations(UP_MIGRATIONS).context("parsing up migrations")?;
    let parsed_down_migrations =
        parse_migrations(DOWN_MIGRATIONS).context("parsing down migrations")?;
    ...
    println!(
        "Migration {n:0>3} (up: {up_descr}, down: {down_descr})
Up:
{up_sql}
Down:
{down_sql}",
...
where
    SkipF: FnMut(&Migration<'static>) -> bool,
    TakeF: FnMut(&Migration<'static>) -> bool,
{
    let confstr =
        format!("Yes, I want to migrate {connstr} from {last_migration:0>3} to {target:0>3}.");
    ...
    transaction
        .batch_execute(&migrations.into_iter().map(|m| m.sql).join("\n"))
        .await
...
pub async fn migrate_to(profile: crate::init::Profile, target: u16) -> anyhow::Result<()> {
    ...
    let last_migration = get_last_migration(&client)
        .await
        .context("getting last migration")?;
    match target.cmp(&last_migration) {
        ...
        Ordering::Greater => {
            ...
            .await
            .context("performing an up migration")?;
        }
        Ordering::Less => {
            migrate(
                &mut client,
            ...
            .await
            .context("performing a down migration")?;
        }
    }
    Ok(())
}
...
pub async fn initialize_db(profile: crate::init::Profile) -> anyhow::Result<()> {
    log::info!(
        "Initializing database {connstr}",
        connstr = &profile.dbconnstr
    );
    let mut client = connect(&profile.dbconnstr)
        .await
    ...
    transaction
        .batch_execute(UP_MIGRATIONS_SQL)
        .await
...
pub async fn print_database_info(profile: crate::init::Profile) -> anyhow::Result<()> {
    let client = connect(&profile.dbconnstr)
        .await
        .context("connecting to database")?;

    let last_migration_opt = get_last_migration(&client)
        .await
