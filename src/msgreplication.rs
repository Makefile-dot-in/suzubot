...
impl Display for ReplicationErrorContext {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::ReplicatingMessage { message, target, thread: None } =>
				write!(f, "logging message {message_link} to {target_mention}",
					   message_link = linkable_message_to_link(*message),
					   target_mention = target.mention()),
			Self::ReplicatingMessage { message, target, thread: Some(thread) } =>
				write!(f, "logging message {message_link} to {target_mention} in {thread_mention}",
					   message_link = linkable_message_to_link(*message),
					   target_mention = target.mention(),
...
fn convey_message_content<'a, 'b>(
	...
	match msg.kind {
		...
		GroupRecipientAddition => {
			embeds.push(simple_embed("➕ Added User(s)", format!("Added {}.", msg.mentions.iter().map(|m| m.mention()).join(", ")), ser::Color::FOOYOO))
		},
		GroupRecipientRemoval => {
			embeds.push(simple_embed("❌ Removed User(s)", format!("Removed {}.", msg.mentions.iter().map(|m| m.mention()).join(", ")), ser::Color::RED))
		},
		GroupCallCreation => {
			embeds.push(simple_embed("📞 Started a Call", "A call was started.", ser::Color::DARK_GREEN))
		},
		GroupNameUpdate => {
			embeds.push(simple_embed("📛 Changed Group Name", format!("The group name was changed to {}", msg.content), ser::Color::TEAL))
		},
		GroupIconUpdate => {
			embeds.push(simple_embed("🖼️ Changed Group Icon", "The group icon was changed.", ser::Color::TEAL))
		},
		PinsAdd => {
			...
			embeds.push(simple_embed("📌 Pinned Message", description, ser::Color::RED))
		},
		MemberJoin => {
			embeds.push(simple_embed("👋 Joined", "Welcome to the server!", ser::Color::FOOYOO))
		},
		NitroBoost | NitroTier1 | NitroTier2 | NitroTier3 => {
			...
			embeds.push(simple_embed("🚀 Boosted Server", description, ser::Color::BLURPLE))
		},
		ChannelFollowAdd => {
			embeds.push(simple_embed("📣 Followed a channel", format!("This channel will now receive updates from {}", msg.content), ser::Color::DARK_RED))
		},
		GuildDiscoveryDisqualified => {
			embeds.push(simple_embed("❌ Discovery Disqualified", "This server was disqualified from Discovery.", ser::Color::RED));
		},
		GuildDiscoveryRequalified => {
			embeds.push(simple_embed("✅ Discovery Requalified", "This server is qualified again for Discovery.", ser::Color::FOOYOO));
		},
		GuildDiscoveryGracePeriodInitialWarning => {
			embeds.push(simple_embed("⚠️ Discovery Disqualification Warning", "This is the initial warning for disqualification from Discovery.", ser::Color::GOLD));
		},
		GuildDiscoveryGracePeriodFinalWarning => {
			embeds.push(simple_embed("⚠️ Discovery Disqualification Warning", "This is the **final** warning for disqualification from Discovery.", ser::Color::GOLD));
		},
		ThreadCreated => {
			let description = match msg.thread {
				Some(thread) => format!("Started a thread named {}: {}.", msg.content, thread.mention()),
				None => format!("Started a thread named {}.", msg.content)
			};

			embeds.push(simple_embed("🧵 Started Thread", description, ser::Color::FADED_PURPLE));
		},
		InlineReply => {
			let description = match referenced {
				Some(msg) => format!("Replied to [this]({}) message.", linkable_message_to_link(msg)),
		...
		ThreadStarterMessage => {
			...
			w.content(msg.content);
			embeds.push(simple_embed("🔰 Thread Starter Message", description, ser::Color::TEAL));
		},
		GuildInviteReminder => {
			embeds.push(simple_embed("⏰ Guild Invite Reminder", "Clyde wants to let you know that this server is too empty.", ser::Color::BLURPLE));
		},
		AutoModerationAction => {
			embeds.push(simple_embed("🤖 Automoderation Action", "You can see the details below.", ser::Color::BLURPLE));
		},
		_ => {
			embeds.push(simple_embed("❓ Unknown", "Unknown message type.", ser::Color::from_rgb(0, 0, 0)));
		},
	}
...
fn attachment_to_attachment_type_or_embed(
	...
	for attachment in attachments.into_iter() {
		let attachment_type_opt = ({ size += attachment.size; size < MAX_DOWNLOAD_SIZE })
			.then_some(())
			.ok_or_else(|| None)
		...
		match attachment_type_opt {
			...
			Err(e) => {
				...
				embeds.push(simple_embed(format!("📎 Attachment: {}", attachment.filename),
