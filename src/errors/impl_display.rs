...
use super::{
    Context, Error, InternalError, LoggedContext, LoggedError, LoggedFrameworkError,
    LoggedMappedWithContext, OptError, WithContext,
};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            SerenityError(ser::Error::Http(httperr)) => match &**httperr {
                ...
                err => write!(f, "Serenity HTTP error: {err}"),
            },
...
impl Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use Error::*;
        match self {
            MissingPermission(permissions) => write!(f, "Missing permissions: {permissions}"),
            ...
            GetLatencyError(e) => write!(f, "{e}"),
            RemindError(e) => write!(f, "{e}"),
            Log(e) => write!(f, "{e}"),
            Internal(_) => write!(f, "Internal error"),
        }
    }
}
...
impl<'a> Display for LoggedContext<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "command {command}, ",
            command = self.0.invocation_string()
        )?;
        ...
        write!(
            f,
            "run by user {author_id} [{author_tag}]",
...
pub(crate) fn withctx_error_logged(
    e: &WithContext<Error>,
) -> LoggedMappedWithContext<'_, Error, LoggedError<'_>> {
