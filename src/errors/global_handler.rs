...
use poise::{ApplicationCommandOrAutocompleteInteraction, FrameworkError};
    ...
    match err {
        ...
        SubcommandRequired { ctx } => {
            let subcommand_string = match ctx.command().subcommands.as_slice() {
                ...
                [cmd1, cmd2] => format!(
                    "Either `{cmd1_name}` or `{cmd2_name}`",
                    cmd1_name = cmd1.name,
                    cmd2_name = cmd2.name
                ),
                [cmds @ .., last_cmd] => format!(
                    "Either `{cmd_names}`, or `{last_cmd_name}`",
            ...
            ctx.say(format!(
                "{command} requires a subcommand: {subcommand_string}",
        ...
        ArgumentParse {
            ...
            ctx.say(format!("Parsing error: {error}")).await.logerr();
        }
        CommandStructureMismatch { ctx, .. } => {
            ctx.say("Command failed due to a bot configuration error.")
                .await
                .logerr();
        }
        CooldownHit {
            ...
            ctx.say(format!(
                "Cooldown not yet expired ({seconds} seconds remaining)",
        ...
        MissingBotPermissions {
            ...
            ctx.say(format!(
                "The bot is missing permissions: {missing_permissions}."
            ))
            .await
            .logerr();
        }
        MissingUserPermissions {
            missing_permissions: Some(perms),
            ctx,
            ..
        } => {
            ctx.say(format!(
                "You need the following permissions to run this command: {perms}"
            ))
            .await
            .logerr();
        }
        MissingUserPermissions {
            ...
            ctx.say(format!(
                "The bot couldn't retrieve your permissions, so to be safe, \
							 you can't run the command, as it requires the {cmd_perms} permission(s).",
                cmd_perms = ctx.command().required_permissions
            ))
            .await
            .logerr();
        }
        ...
        DmOnly { ctx, .. } => {
            ctx.say("This command may only be used in DMs.")
                .await
                .logerr();
        }
        NsfwOnly { ctx, .. } => {
            ctx.say("This command may only be used in NSFW channels.")
                .await
                .logerr();
        }
        ...
        UnknownInteraction {
            interaction: ApplicationCommandOrAutocompleteInteraction::Autocomplete(_),
            ..
        } => {}
        _ => {}
    };
